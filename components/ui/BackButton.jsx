...
export default function BackButton({ fallback }) {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.96 }}
      onClick={handleBack}
      className="fixed top-[88px] left-6 z-30 hidden md:flex items-center gap-2 rounded-button bg-scrim/30 px-4 py-2.5 text-white/90 text-xs font-medium tracking-[0.15em] uppercase shadow-lg backdrop-blur-md border border-white/15 hover:bg-scrim/50 hover:border-white/30 hover:text-white transition duration-300"
    >
